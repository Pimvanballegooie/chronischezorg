...
# -*- coding: utf-8 -*-
"""
Sync protocollen vanuit Google Docs — Chronische Zorg Netwerk (chronischezorg.net)
...
# ══════════════════════════════════════════════════════════════════
# SITE-INSTELLINGEN — Chronische Zorg Netwerk
# ══════════════════════════════════════════════════════════════════
SITE_URL     = "https://chronischezorg.net"
SITE_NAAM    = "Chronische Zorg Netwerk"
LOGO_BESTAND = "CZ_Logo.png"
...
# Negen specialisaties als zones (kleuren per groep uit het CZ-logo)
ZONES = {
    "longklachten":    "Longklachten",
    "etalagebenen":    "Etalagebenen",
    "hartrevalidatie": "Hartrevalidatie",
    "oncologie":       "Oncologische revalidatie",
    "chronische-pijn": "Chronische pijn",
    "vermoeidheid":    "Chronische vermoeidheid",
    "diabetes":        "Diabetes en bewegen",
    "reuma-artrose":   "Reuma, artrose en botgezondheid",
    "operatie":        "Voorbereiding op operatie",
}
ZONE_ICONS = {
    "longklachten": "🫁", "etalagebenen": "🚶", "hartrevalidatie": "❤️",
    "oncologie": "🎗️", "chronische-pijn": "⚡", "vermoeidheid": "😴",
    "diabetes": "🩸", "reuma-artrose": "❤️", "operatie": "📋",
}
ZONE_KLEUREN = {
    "longklachten": "#C43A3A", "etalagebenen": "#C43A3A", "hartrevalidatie": "#C43A3A",
    "oncologie": "#2E86C1", "chronische-pijn": "#2E86C1", "vermoeidheid": "#2E86C1",
    "diabetes": "#3BAE8C", "reuma-artrose": "#3BAE8C", "operatie": "#3BAE8C",
}
...
def haal_doc_tekst(doc_id: str) -> str | None:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "ChronischeZorg-Sync/1.0"})
...
def header_html(root_pad: str = "") -> str:
  ...
  <div class="header-inner">
    <a href="{root_pad}index.html" class="logo">
      ...
      <div class="logo-text"><span>Chronische</span> Zorg</div>
    </a>
...
def bouw_protocol_pagina(protocol: dict, niveau: str, inhoud_html: str) -> str:
    ...
    zone_icon = ZONE_ICONS.get(zone, "❤️")
    ...
    if is_patient:
      ...
      ℹ️ Deze uitleg hoort bij een behandelprotocol van het Chronische Zorg Netwerk en is geen vervanging
...
window.addEventListener('beforeunload', () => speechSynthesis.cancel());
      ...
      ⚕️ Klinische versie voor aangesloten therapeuten van het Chronische Zorg Netwerk. Toepassing vraagt
...
def bouw_overzicht(protocollen: list, previews: dict) -> str:
    ...
    if protocollen:
      ...
      <p style="color:var(--text-muted);font-size:0.9rem;max-width:440px;margin:0 auto">
        Onze expertteams werken aan de eerste behandelprotocollen voor chronische aandoeningen.
...
<head>
  ...
  <meta name="description" content="Behandelprotocollen van het {SITE_NAAM}: COPD, hartrevalidatie, chronische pijn, diabetes en meer. Elk protocol in een leesbare versie voor patiënten en een klinische versie voor therapeuten." />
  <style>
    {basis_css()}
    .hero {{ background: linear-gradient(160deg, #3BAE8C 0%, #2E86C1 100%); position: relative; color: white; text-align: center; padding: 56px 24px 48px; }}
...
<div class="pagina">
  <div class="zoek-rij">
    <input type="text" class="zoek-input" id="zoek-input" placeholder="🔍 Zoek bijv. COPD, etalagebenen, chronische pijn..." oninput="filterProtocollen()" />
  </div>
